        ...
        <div class="max-w-7xl mx-auto px-4 flex flex-col md:flex-row gap-8">
            ...
            <div class="w-1/4 min-w-[200px] hidden md:block">
                <div class="sticky top-24 h-[calc(100vh-120px)] overflow-y-auto pr-4 border-r border-gray-200">
                    ...
                    @endphp
                    <a href="#" data-id="{{ $category->cid }}" onclick="filterByCategory(event, this)"
            ...
            <div class="flex-1 w-full">
                <div class="mb-8 pb-6 border-b border-gray-200">
                    ...
                    <form id="searchForm" action="{{ route('exploreplaces') }}" method="GET" class="flex flex-wrap gap-3 w-full">
                ...
                <div id="placesGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden group">
                        <div class="w-full h-56 overflow-hidden relative">
                            ...
                            <img src="{{ \Illuminate\Support\Facades\Storage::disk('s3')->url($place->main_image) }}" alt="{{ $place->name }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                            @elseif(isset($place->images[0]))
                            <img src="{{ \Illuminate\Support\Facades\Storage::disk('s3')->url($place->images[0]) }}" alt="{{ $place->name }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                            @else
                        ...
                        <div class="p-5 flex flex-col flex-1">
                           ...
                           <div class="mt-auto pt-4 border-t border-gray-100">
                                <a href="{{ route('exploreplaces.show', $place->id) }}" class="block text-center bg-blue-600 hover:bg-blue-700 text-white font-medium px-4 py-2 rounded-xl transition duration-300 w-full text-sm">
                                    View Details
                                </a>
                            </div>
                        </div>
                ...
                <div id="noPlacesMessage" class="{{ $exploreplaces->isEmpty() ? 'block' : 'hidden' }} text-center py-16">
                    <h3 class="text-xl text-gray-500 font-medium">No places found matching your search or filters.</h3>
                </div>
        ...
        function fetchUpdatedPlaces() {
            ...
            fetch(url, {
                ...
                .then(html => {
                    ...
                    const newNoPlaces = doc.querySelector('#noPlacesMessage');
                    const currentNoPlaces = document.querySelector('#noPlacesMessage');

                    if (newGrid.children.length === 0) {
                        currentNoPlaces.classList.remove('hidden');
                        currentNoPlaces.classList.add('block');
                    } else {
                        currentNoPlaces.classList.add('hidden');
                        currentNoPlaces.classList.remove('block');
                    }
                    ...
                    document.querySelectorAll('.category-item').forEach(el => {
                        ...
                        if (id !== undefined) {
                            ...
                            el.classList.toggle('hover:bg-blue-50', !isSelected);
                        }
                    });
